...
public class HandBrakeService : IHandBrakeService
{
	...
	private readonly IScanCacheProvider _scanCache;
		...
		IScanCacheProvider scanCache)
	{
		...
		_resolvedScanOutputPath = Path.IsPathRooted(_options.ScanOutputPath)
			...
			: Path.Combine(_options.ContentRootPath, _options.ScanOutputPath);
	...
	public async Task<HandBrakeScanResult> ScanAsync(string inputPath, bool forceRescan = false, CancellationToken ct = default)
	{
		...
		// Check cache unless a force rescan was requested (24-hour TTL)
		if (!forceRescan)
		{
			var cachedJson = await _scanCache.GetCachedScanJsonAsync(inputPath, ct).ConfigureAwait(false);
			if (!string.IsNullOrWhiteSpace(cachedJson))
			{
				_logger.LogInformation("Returning cached scan result for {InputPath}", inputPath);
				var cachedTitles = HandBrakeParser.ParseScan(cachedJson);
		...
		// Persist to cache (replace any existing entry for this path)
		try
		{
			await _scanCache.SaveScanAsync(inputPath, result.StandardOutput, ct).ConfigureAwait(false);
		}
		catch (Exception ex)
		{
